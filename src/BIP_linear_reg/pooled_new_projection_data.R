...
path <- "~/../../../../project/biocomplexity/sdad/projects_data/project_data/usda/rural_broadband/BIP_linear_models/"

dataset <- readRDS(paste0(path, "housing_BIP_060622.RDS"))

# drop deeds before 2005 
dataset <- dataset %>% filter(sale_year>=2005)
...
model_tract = try(lm(log(dataset$sale_price) ~ -1 + Xmat + substr(dataset$geoid_blk,1,11)-1), silent = T)
