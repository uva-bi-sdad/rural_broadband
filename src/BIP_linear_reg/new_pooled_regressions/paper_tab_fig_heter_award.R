# Heterogenous Effects plots and tables: Award amount
...
lm_ftth <- readRDS("BIP_linear_reg/lm_fe_quant1.RDS")
lm_wire <- readRDS("BIP_linear_reg/lm_fe_quant2.RDS")
lm_dsl <- readRDS("BIP_linear_reg/lm_fe_quant3.RDS")
...
lmfe_ftth <- readRDS("BIP_linear_reg/lmfe_quant1_robust10.RDS")
lmfe_wire <- readRDS("BIP_linear_reg/lmfe_quant2_robust10.RDS")
lmfe_dsl <- readRDS("BIP_linear_reg/lmfe_quant3_robust10.RDS")

lmfema_ftth <- as.data.frame(lmfe_ftth[c(11:15), c(1,2,4)])
...
lmfema_wire <- as.data.frame(lmfe_wire[c(11:15), c(1,2,4)])
...
lmfema_dsl <- as.data.frame(lmfe_dsl[c(11:15), c(1,2,4)])
...
match_ftth <- fread("BIP_linear_reg/quant1_match.csv")
match_wire <- fread("BIP_linear_reg/quant2_match.csv")
match_dsl <- fread("BIP_linear_reg/quant3_match.csv")
...
shapes <- c('0-33 %-tile' = 16, '33-66 %-tile' = 15, "66-100 %-tile" = 17)

ggplot(data=plotmodels_tech) + 
  geom_pointrange(aes(x=years, y=est1, ymin=upper1, ymax=lower1, shape="0-33 %-tile"), 
                  color="black",position = position_nudge(x = -0.1)) + 
  geom_pointrange(aes(x=years, y=est2, ymin=upper2, ymax=lower2, shape="33-66 %-tile"), 
                  color="black", position = position_nudge(x = 0.1)) + 
  geom_pointrange(aes(x=years, y=est3, ymin=upper3, ymax=lower3, shape="66-100 %-tile"), 
                  color="black") + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/lm_awardplot.png",height=4.5,width=6)

####################
# FIG 6b
####################

lmma_ftth <- as.data.frame(lmfe_ftth[11:15,c(1,5,6)])
rownames(lmma_ftth) <- yearnames
names(lmma_ftth) <- c("est1","lower1","upper1")
lmma_ftth$years <- as.character(yearnames)

lmma_wire <- as.data.frame(lmfe_wire[11:15,c(1,5,6)])
rownames(lmma_wire) <- yearnames
names(lmma_wire) <- c("est2","lower2","upper2")

lmma_dsl <- as.data.frame(lmfe_dsl[11:15,c(1,5,6)])
...
ggplot(data=plotmodels_tech) + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/lmma_awardplot.png",height=4.5,width=6)
...
ggplot(data=plotmodels_tech) + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/ma_awardplot_fig15.png",height=4.5,width=6)
