...
# award quantiles 
#levels.x <- names(table(as.character(quant1$quantiles), useNA = "ifany"))
#if(levels.x != "" & !is.na(levels.x) & length(levels.x) != 1){
#Xmat <-cbind(Xmat,sapply(levels.x[-1], function(x){
#    as.numeric(as.character(quant1$quantiles)==x)
#  })) 
...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 5 : (5 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(quant1$bip) * Xmat[, 5 : (5 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 5 : (5 + id.yr - 2)], 2, function(k) as.numeric(quant1$bip) * k))
...
dataset_f <- quant1[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_quant1_robust10.RDS")
...
# award quantiles 
#levels.x <- names(table(as.character(quant2$quantiles), useNA = "ifany"))
#if(levels.x != "" & !is.na(levels.x) & length(levels.x) != 1){
#  Xmat <-cbind(Xmat,sapply(levels.x[-1], function(x){
#    as.numeric(as.character(quant2$quantiles)==x)
#  })) 
#  if(levels.x != "") cnames <- c(cnames,paste("quantiles:",levels.x[-1],sep=""))
#}
...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 5 : (5 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(quant2$bip) * Xmat[, 5 : (5 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 5 : (5 + id.yr - 2)], 2, function(k) as.numeric(quant2$bip) * k))
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 1)],"xbip", sep = "")) 
    }
  }else{
    if(is.null(ncol(Xmat[, 5 : (5 + id.yr - 2)]))){
...
dataset_f <- quant2[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_quant2_robust10.RDS")
...
# award quantiles 
#levels.x <- names(table(as.character(quant3$quantiles), useNA = "ifany"))
#if(levels.x != "" & !is.na(levels.x) & length(levels.x) != 1){
#  Xmat <-cbind(Xmat,sapply(levels.x[-1], function(x){
#    as.numeric(as.character(quant3$quantiles)==x)
#  })) 
#  if(levels.x != "") cnames <- c(cnames,paste("quantiles:",levels.x[-1],sep=""))
#}
...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 5 : (5 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(quant3$bip) * Xmat[, 5 : (5 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 5 : (5 + id.yr - 2)], 2, function(k) as.numeric(quant3$bip) * k))
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 1)],"xbip", sep = "")) 
    }
  }else{
    if(is.null(ncol(Xmat[, 5 : (5 + id.yr - 2)]))){
...
dataset_f <- quant3[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_quant3_robust10.RDS")
