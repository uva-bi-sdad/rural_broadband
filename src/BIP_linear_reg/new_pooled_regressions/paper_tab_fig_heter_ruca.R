# Heterogenous Effects plots: RUCA
...
lm_ftth <- readRDS("BIP_linear_reg/lm_fe_metro.RDS")
lm_wire <- readRDS("BIP_linear_reg/lm_fe_micro.RDS")
lm_dsl <- readRDS("BIP_linear_reg/lm_fe_rural.RDS")
...
wire <- as.data.frame(lm_wire[[1]][c(19:23), c(2,3,5)])
...
dsl <- as.data.frame(lm_dsl[[1]][c(19:23), c(2,3,5)])
...
lmfe_ftth <- readRDS("BIP_linear_reg/lmfe_metro_robust10.RDS")
lmfe_wire <- readRDS("BIP_linear_reg/lmfe_micro_robust10.RDS")
lmfe_dsl <- readRDS("BIP_linear_reg/lmfe_rural_robust10.RDS")

lmfema_ftth <- as.data.frame(lmfe_ftth[c(13:17), c(1,2,4)])
lmfema_ftth$`Std. Error` <- round(as.numeric(lmfema_ftth$`Std. Error`), 3)
lmfema_ftth$Estimate <- round(as.numeric(lmfema_ftth$Estimate), 3)
...
lmfema_ftth <- rbind(lmfema_ftth, lmfe_ftth["Std. Error"][nrow(lmfe_ftth["Std. Error"]),])

lmfema_wire <- as.data.frame(lmfe_wire[c(13:17), c(1,2,4)])
lmfema_wire$`Std. Error` <- round(as.numeric(lmfema_wire$`Std. Error`), 3)
lmfema_wire$Estimate <- round(as.numeric(lmfema_wire$Estimate), 3)
...
lmfema_wire <- rbind(lmfema_wire, lmfe_wire["Std. Error"][nrow(lmfe_wire["Std. Error"]),])

lmfema_dsl <- as.data.frame(lmfe_dsl[c(13:17), c(1,2,4)])
lmfema_dsl$`Std. Error` <- round(as.numeric(lmfema_dsl$`Std. Error`), 3)
lmfema_dsl$Estimate <- round(as.numeric(lmfema_dsl$Estimate), 3)
...
lmfema_dsl <- rbind(lmfema_dsl, lmfe_dsl["Std. Error"][nrow(lmfe_dsl["Std. Error"]),])
...
match_ftth <- fread("BIP_linear_reg/metro_match.csv")
match_wire <- fread("BIP_linear_reg/micro_match.csv")
match_dsl <- fread("BIP_linear_reg/rural_match.csv")
...
wire_10 <- as.data.frame(lm_wire[[1]][19:23,c(2,6,7)])
rownames(wire_10) <- yearnames
names(wire_10) <- c("est2","lower2","upper2")

dsl_10 <- as.data.frame(lm_dsl[[1]][19:23,c(2,6,7)])
...
shapes <- c('Metro' = 16, 'Micro' = 15, "Rural" = 17)

ggplot(data=plotmodels_tech) + 
  geom_pointrange(aes(x=years, y=est1, ymin=upper1, ymax=lower1, shape="Metro"), 
                  color="black",position = position_nudge(x = -0.1)) + 
  geom_pointrange(aes(x=years, y=est2, ymin=upper2, ymax=lower2, shape="Micro"), 
                  color="black", position = position_nudge(x = 0.1)) + 
  geom_pointrange(aes(x=years, y=est3, ymin=upper3, ymax=lower3, shape="Rural"), 
                  color="black") + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/lm_rucaplot.png",height=4.5,width=6)

#############
# FIG 7b
#############

lmma_ftth <- as.data.frame(lmfe_ftth[13:17,c(1,5,6)])
rownames(lmma_ftth) <- yearnames
names(lmma_ftth) <- c("est1","lower1","upper1")
lmma_ftth$years <- as.character(yearnames)

lmma_wire <- as.data.frame(lmfe_wire[13:17,c(1,5,6)])
rownames(lmma_wire) <- yearnames
names(lmma_wire) <- c("est2","lower2","upper2")

lmma_dsl <- as.data.frame(lmfe_dsl[13:17,c(1,5,6)])
...
ggplot(data=plotmodels_tech) + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/lmma_rucaplot.png",height=4.5,width=6)
...
ggplot(data=plotmodels_tech) + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/ma_rucaplot_fig17.png",height=4.5,width=6)
