...
# get the tract ID
match_data$geoid_tract <- substr(match_data$geoid_blk,1,11)
match_data$geoid_tract <- as.character(as.numeric(match_data$geoid_tract))
...
match_data <- left_join(match_data, elig_df, by=c("geoid_tract" = "GEOID"))

############################################
# NO PROVIDERS WITH MORE THAN 3MBTS IN 2011 
############################################

provider3 <- match_data %>% filter(fcc2011_providers_3 == 0)
...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 5 : (5 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(provider3$bip) * Xmat[, 5 : (5 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 5 : (5 + id.yr - 2)], 2, function(k) as.numeric(provider3$bip) * k))
...
provider3$geoid_tr <- substr(provider3$geoid_blk,1,11)
...
dataset_f <- provider3[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_provider3_robust10.RDS")
