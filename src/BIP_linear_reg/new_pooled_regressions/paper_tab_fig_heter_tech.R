# Heterogenous Effects plots: Tech type 
...
lmfe_ftth <- readRDS("BIP_linear_reg/lmfe_ftth_robust10.RDS")
lmfe_wire <- readRDS("BIP_linear_reg/lmfe_wireless_robust10.RDS")
lmfe_dsl <- readRDS("BIP_linear_reg/lmfe_dsl_robust10.RDS")

lmfema_ftth <- as.data.frame(lmfe_ftth[c(9:13), c(1,2,4)])
...
lmfema_dsl <- as.data.frame(lmfe_dsl[c(9:13), c(1,2,4)])
...
lmma_ftth <- as.data.frame(lmfe_ftth[9:13,c(1,5,6)])
...
lmma_dsl <- as.data.frame(lmfe_dsl[9:13,c(1,5,6)])
...
ggplot(data=plotmodels_tech) + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/ma_techplot_fig13.png",height=4.5,width=6)
