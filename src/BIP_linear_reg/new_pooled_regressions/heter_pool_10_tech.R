...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 3 : (3 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(ftth_df$bip) * Xmat[, 3 : (3 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 3 : (3 + id.yr - 2)], 2, function(k) as.numeric(ftth_df$bip) * k))
...
dataset_f <- ftth_df[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_ftth_robust10.RDS")

###########################
# WIRELESS
###########################

wireless_df <- match_data %>% filter(wireless == 1)
...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 3 : (3 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(wireless_df$bip) * Xmat[, 3 : (3 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 3 : (3 + id.yr - 2)], 2, function(k) as.numeric(wireless_df$bip) * k))
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 1)],"xbip", sep = "")) 
    }
  }else{
    if(is.null(ncol(Xmat[, 3 : (3 + id.yr - 2)]))){
...
dataset_f <- wireless_df[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_wireless_robust10.RDS")
...
if(id.yr != 1){
  if(!flag.in){
    if(is.null(ncol(Xmat[, 3 : (3 + id.yr - 2)]))){
      Xmat <- cbind(Xmat, as.numeric(dsl_df$bip) * Xmat[, 3 : (3 + id.yr - 2)])
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 2)],"xbip", sep = ""))
    }else{
      Xmat <- cbind(Xmat, apply(Xmat[, 3 : (3 + id.yr - 2)], 2, function(k) as.numeric(dsl_df$bip) * k))
      cnames <- c(cnames, paste(cnames[1 : (id.yr - 1)],"xbip", sep = "")) 
    }
  }else{
    if(is.null(ncol(Xmat[, 3 : (3 + id.yr - 2)]))){
...
dataset_f <- dsl_df[close_filter,]; Xmat_f <- Xmat[close_filter,]
...
saveRDS(model_res,file="BIP_linear_reg/lmfe_dsl_robust10.RDS")
