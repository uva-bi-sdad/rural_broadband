...
ftth <- as.data.frame(lm_ftth[[1]][c(19:23), c(2,3,5)])
...
lmfe_ftth <- readRDS("BIP_linear_reg/fe_matched_metro.RDS")
lmfe_wire <- readRDS("BIP_linear_reg/fe_matched_micro.RDS")
lmfe_dsl <- readRDS("BIP_linear_reg/fe_matched_rural.RDS")
...
lmfema_ftth <- as.data.frame(lmfe_ftth[[1]][c(13:17), c(2,3,5)])
...
lmfema_wire <- as.data.frame(lmfe_wire[[1]][c(13:17), c(2,3,5)])
...
lmfema_dsl <- as.data.frame(lmfe_dsl[[1]][c(13:17), c(2,3,5)])
...
ftth_10 <- as.data.frame(lm_ftth[[1]][19:23,c(2,6,7)])
...
ggplot(data=plotmodels_tech) + 
  ...
  labs(#title = "Estimated Coefficients on the Interactions between BIP and 2-year Period Dummies", 
    ...
    y = "Estimate, %")+
  theme_classic()
ggsave("BIP_linear_reg/ma_rucaplot.png",height=4.5,width=6)
